 ...
 */
export function hash(cell: Cell) {
...
export class CellModel {
   ...
   */
  private readonly _state: Map<number,[number,number]> = new Map();
  ...
  private _cellWillChange(cell: Cell, alive: boolean) {
    ...
    for ( const offset of this._search_pattern ) {
      ...
      if ( this._state.has(hash(neighbor)) ) neighbor_count++;
    }
  ...
  private _toggleCells(cells: Cell[]) {
    const state_changes = new Map(cells.map((cell: Cell) => [hash(cell), cell]));
    for ( const entry of state_changes ) {
      const cell_hash = entry[0], cell = entry[1];
      if (! this._state.delete(cell_hash) ) this._state.set(cell_hash, cell);
    }
  }
  ...
  _nextState() {
    ...
    for ( const entry of this._state.entries() ) {
      const living_hash = entry[0], living_cell = entry[1];
      if ( used.has(living_hash) ) continue;
      used.add(living_hash);
      ...
      for ( const offset of this._search_pattern ) {
        ...
        const neighbor_hash = hash(neighbor_cell);
        if ( used.has(neighbor_hash) ) continue;
        used.add(neighbor_hash);
        if ( this._cellWillChange(neighbor_cell, this._state.has(neighbor_hash)) ) {
